...
const MasterUpdate = ({toggleModal, data}) => {
    ...
    if (errors && Object.keys(errors).length === 0) {
        ...
        const  OriginalData = {
            ...formData,
            selectedTimeSlot: timeslot?.map(option => option.value).join(', '),
        ...
        const apiUrl =  `${API_URL}/monthly-service/master-update/${data.orderNo}`

        axios.put(apiUrl, OriginalData, {'Content-Type': 'multipart/form-data'})
            .then( async (response) => {
                setIsLoading(false)
                if (response.status === 200) {
